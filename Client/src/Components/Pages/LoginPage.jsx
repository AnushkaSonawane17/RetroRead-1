...
const LoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      /*
        Store the information returned by backend.
        
        Your App.jsx checks for "token".
        We are using the user's MongoDB _id as the token
        for now because we are not using JWT yet.
      */
    ...
    <div className="min-h-screen w-full relative flex items-center justify-center px-4 py-10 overflow-hidden">
      <style>{`
        ...
        .input-field:focus-within {
          ...
          border-color: #D8472F;
          box-shadow:
            0 0 0 4px rgba(216,71,47,0.08),
            0 8px 24px rgba(216,71,47,0.06);
        ...
        .glass-card {
          ...
          border: 1px solid rgba(255,255,255,0.5);
          box-shadow:
            0 40px 80px -30px rgba(30,42,66,0.4),
      ...
      {/* Login Card */}
      <div
        className={`glass-card p-8 md:p-10 transition-all duration-800 ${
          mounted
      ...
      >
        ...
        <div className="flex flex-col items-center text-center mb-8">
          ...
          <span className="font-display font-semibold text-xl text-[#1E2A42] tracking-wide mt-3">
            RetroRead
          </span>
          ...
          <div className="inline-flex items-center gap-1.5 rounded-full border border-[#D8472F]/20 bg-white/40 px-3 py-1 text-[10px] tracking-wide text-[#5B6478] mt-2">
            <Sparkles size={11} className="text-[#D8472F]" />
            A new line, every visit
          </div>
        </div>

        {/* Heading */}
        <h2 className="font-display text-2xl font-bold text-[#1E2A42] text-center">
          Welcome Back
        </h2>

        <p className="text-[#5B6478] text-sm mt-1.5 mb-7 text-center">
        ...
        <form onSubmit={handleSubmit}>
          ...
          {/* Remember / Forgot */}
          <div className="flex items-center justify-between text-sm px-1 mt-4 mb-6">
            <label className="flex items-center gap-2 text-[#5B6478] cursor-pointer select-none group">
              <input
                ...
                className="h-4 w-4 rounded border-[#D9C7A3] accent-[#D8472F] cursor-pointer"
              />
            ...
            <Link
              to="/forgot-password"
              className="text-[#D8472F] font-medium hover:underline hover:text-[#B23522] transition"
            >
              Forgot Password?
            </Link>
          </div>
          ...
          <div className="flex items-center gap-4 my-6">
            ...
            <span className="text-xs text-[#5B6478] font-medium whitespace-nowrap">
              or continue with
            </span>
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            {[
              ...
              <button
                key={social.name}
                type="button"
                className={`social-btn flex items-center justify-center text-[#1E2A42] font-display font-semibold text-lg ${social.color}`}
          ...
          <p className="text-center text-sm text-[#5B6478]">
            ...
            <Link
              to="/register"
              className="text-[#D8472F] font-semibold hover:underline hover:text-[#B23522] transition"
            >
              Register Now
            </Link>
          </p>
        ...
        <p className="font-quote italic text-center text-sm text-[#5B6478]/60 mt-6 leading-relaxed">
          "{quote.text}"{" "}
          <span className="not-italic text-xs text-[#8A7F6B]">
            — {quote.author}
          </span>
        </p>
      </div>
