...
import { Search, X, Sparkles, Star, User, BookOpen, Heart, ArrowUpDown, Tag, Store } from 'lucide-react';
...
const categories = ['All', ...Array.from(new Set(defaultBooks.map((b) => b.category)))];
...
const MarketplacePage = () => {
  ...
  useEffect(() => {
    const role = localStorage.getItem('userType') || localStorage.getItem('userRole');
    setUserRole(role);
    
    // ✅ Load books from localStorage
    const savedBooks = JSON.parse(localStorage.getItem('marketplaceBooks') || '[]');
    if (savedBooks.length > 0) {
      setAllBooks([...defaultBooks, ...savedBooks]);
    }
  }, []);

  // ✅ Listen for storage changes (when new book is added)
  useEffect(() => {
 ...
 const handleClaim = (bookId) => {
  ...
  const handleSell = (bookId) => {
    navigate(`/sell-book?bookId=${bookId}`);
  };

  // ✅ Handle Offer a Book button click - NAVIGATES TO SELL PAGE
  ...
  return (
    <div className="min-h-screen w-full bg-[#F6EFE3] text-[#1E2A42] py-10 relative overflow-hidden">
      <style>{`
        ...
        .book-card img { transition: transform 0.5s ease; }
        .book-card:hover img { transform: scale(1.05); }
        .book-card .corner { opacity: 0; transition: opacity 0.25s ease; }
        .book-card:hover .corner { opacity: 1; }
        ...
        .btn-claim:hover { background: #B8934F; transform: translateY(-2px); }
        ...
        .btn-sell:hover { background: #573C6B; transform: translateY(-2px); }
        ...
        <div className="relative bg-[#FFFBF3] rounded-2xl p-6 border border-[#E2D5BC] shadow-[0_8px_20px_-14px_rgba(30,42,66,0.3)] overflow-hidden">
          ...
          <div className="relative flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              {/* ✅ Offer a Book Button - Navigates to /sell-book */}
              <button 
                  ...
                  <div className="p-4 flex-1 min-w-0 flex flex-col">
                    ...
                    <div className="flex justify-between items-center mt-auto pt-3">
                      ...
                      {userRole === 'seller' ? (
                        ...
                        >
                          <Store size={12} className="inline mr-1" /> Sell
                        </button>
                      ) : (
                        <button 
                          className="btn-claim"
